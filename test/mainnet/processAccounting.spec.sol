...
contract ProcessAccountingTest is BaseIntegrationTest {
    ...
    function test_processAccounting_withDonation(uint256 depositAmount, uint256 donationPercent) public {
        ...
        vault.depositAsset(MC.WBNB, depositAmount, address(this));

        // Now donate the additional amount
        deal(MC.WBNB, address(this), donationAmount);
        IERC20(MC.WBNB).transfer(address(vault), donationAmount);
    ...
    function test_processAccounting_with_zero_gains() public {
        deal(MC.WBNB, address(this), 100000 ether);
        IERC20(MC.WBNB).approve(address(vault), 100000 ether);
        vault.depositAsset(MC.WBNB, 100000 ether, address(this));
        uint256 totalAssetsBefore = vault.totalAssets();
        uint256 totalSupplyBefore = vault.totalSupply();
    ...
    function test_processAccounting_with_a_loss() public {
        ...
        deal(MC.WBNB, minter, 100000 ether);
        vm.prank(minter);
        IERC20(MC.WBNB).approve(address(mockAsset), 100000 ether);
        vm.prank(minter);
    ...
    function test_processAccounting_depositAndDonation_Successive_toggles() public {
        ...
        deal(MC.WBNB, address(this), secondDepositAmount);
        IERC20(MC.WBNB).approve(address(vault), secondDepositAmount);
        vault.depositAsset(MC.WBNB, secondDepositAmount, address(this));
