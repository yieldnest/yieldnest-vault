...
contract GenerateVaultUpgradeTxData is BaseScript {
    ...
    function run() external {
        ...
        // Get withdrawer from vault assets; use the getter that does not revert if absent
        data.withdrawer = VaultVerification.safeGetWithdrawer(vault);
        ...
        if (data.newWithdrawerImplementation != address(0) && address(data.withdrawer) != (address(0))) {
