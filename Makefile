...
cover   :;  FOUNDRY_PROFILE=default forge coverage --report lcov --report-file default_coverage.info && \
            FOUNDRY_PROFILE=mainnet forge coverage --report lcov --report-file mainnet_coverage.info && \
            lcov --ignore-errors inconsistent -a default_coverage.info -a mainnet_coverage.info -o lcov.info && \
			lcov --remove lcov.info 'test/*' 'script/*' -o lcov.info && \
